...
void ADC_TempSensor_Init(void)
{
    ...
    // set the CCR ccr mode to 1
    // RCC->CFGR2 |= RCC_CFGR2_ADCPRE12_DIV2;
    // ADC1_2_COMMON->CCR &= 0x11UL << ADC_CCR_CKMODE_Pos;
    // ADC1_2_COMMON->CCR |= 0x10UL << ADC_CCR_CKMODE_Pos;
    RCC->AHBENR |= RCC_AHBENR_ADC12EN;
...
float Convert_NTC_Temperature(uint16_t adc_value) {
    ...
    if (adc_value == 0 || adc_value == (ADC_RESOLUTION - 1)) {
        return -273.15; // Invalid temperature (absolute zero)
    }
